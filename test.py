    ...
    for name in ["bogiefile", "bogiefile.yaml"]:
        ...
        try:
            ...
            for env in data["environments"]:
                ...
                aws_account = inputs.get('aws_account_name', 'N/A')
                resources.append({
...
CSS = """
        lines.append('<table><thead><tr><th>Environment</th><th>Gear</th><th>Resource Type</th><th>Region(s)</th><th>AWS Account</th></tr></thead><tbody>')
        for br in bogie_resources:
            lines.append("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>" % (br["environment"], br["gear"], br["resource_type"], br["regions"], br["aws_account_name"]))
...
def generate_report(data, bogie_resources=None):
    ...
    if bogie_resources:
        ...
        lines.append("<th>Environment</th><th>Gear</th><th>Resource Type</th><th>Region(s)</th>")
